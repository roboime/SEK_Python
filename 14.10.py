...
velocidade = 400    
deltaDir = 160         # delta de velocidade ao ajeitar caminho
deltaEsq = 110
fator = 1
v_curva = 300       # velocidade em curvas


pos_dir = 455     # as curvas
pos_esq = pos_dir
...
def aprender(aprendizado):
    print("aprendeu")
    aprendizado[robot.contador] = atribuiCor(robot.corAntiga)
...
def plaza():
    ...
    while corCheck.value() in branco:
        saindoRetoPlaza()
    if corCheck.value() not in branco:
        print("saindo do preto")
        recuar();    recuar();    recuar()
        recuar();    recuar();    recuar()
        meiaVolta()
        saindoReto()
...
def vendoCor():
    corAtual = atribuiCor(corCheck.value())                             # corAtual eh a cor vista no ladrilho
...
def atiraBonecos():
    motorCatapulta.run_to_rel_pos(position_sp = catapulta_pos, speed_sp = garra_speed)
    ...
    if robot.ida > 1:
        if robot.ida % 2 == 0:
            if ultrassom.value() < dist_bonecos and robot.bonecos < 2:
                ...
                interpretaCor(corCheck.value())
        if robot.ida % 2 == 1:
            if ultrassom.value() < dist_bonecos:
                pegaBonecos()
                if robot.bonecos >= 2:
                    meiaVolta()
                    robot.ida += 1
